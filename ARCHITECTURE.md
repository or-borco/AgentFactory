...
1. **Source control (GitHub)** = *workspace provider + review surface*. GitHub App (not PAT): per-org install,
   ...
   `agent/<task-ref>-<title-slug>-<token>` branch (`sessionBranchName`; `token` is a random suffix stored on the
   session, folded in so the branch stays unique even if the human-readable task ref/title — unique only within
