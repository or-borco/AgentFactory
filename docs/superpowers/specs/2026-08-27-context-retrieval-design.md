...
- **Manual pinning, per-agent enable/disable, and user-tunable `k`/floor.** The right defaults are not known yet; tuning waits on the PR 7 numbers.
...
- **Not asserted in CI:** retrieval quality, following the existing convention for judge quality (`run-eval.spec.ts:11`). PR 7 measures it out-of-band.
...
- **Local embedding quality is unmeasured until PR 7.** If `bge-small` proves too weak, the swap is a `vector(N)` migration plus a full re-embed — cheap in wall-clock, because originals are kept, but not a config flip.
- **The layer ordering is reasoned, not measured.** It follows the rule the ordering experiment established, but that experiment had no retrieved layer in it.
