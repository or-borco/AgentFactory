...
- **Labeled as untrusted, descriptive content, not instruction.** `worker.ts` wraps a non-empty map in a heading that says it's auto-generated and describes the codebase — not instructions — before handing it to `composeSystemPrompt`. The map's content comes from an agent exploring an arbitrary repo with full tool access, so a poisoned README or config file could otherwise get cached and re-presented as if it were platform-authored instruction to every future run against that commit.
