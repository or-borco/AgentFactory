...
- **Extracting image and PDF attachments** into something the agent can actually read (OCR, vision, or a PDF-to-text step). Task 7 makes the agent *aware* such attachments exist; it does not make their content legible. This is exactly the gap `text-extract.ts` already names for PDF/.docx uploads generally — Jira attachments hit the same wall, not a new one. **Update (2026-09-15):** for images specifically, this line is now only half true — `docs/superpowers/specs/2026-09-11-task-context-image-uploads-design.md` (merged after this plan shipped) made JPEG/PNG attachments materialize as real, viewable files in the sandbox (no OCR/vision — the agent still can't read pixels — but it's no longer stuck at a name-only "unavailable" entry either). PDF/.docx are still exactly as described here.
