...
## Post-final-review addendum (added after Task 10)

The final whole-branch review flagged that detecting a review run from a PR link in
`task.description` alone — the design spec's original mechanism — lets any task that merely
*mentions* a PR URL (e.g. "fix the regression from PR #300") silently become a review run: it
stops running as a normal dev/chat task and instead posts a real GitHub review on that PR, or
hard-fails if the org's GitHub connection can't see it. Confirmed with the user (2026-09-16): the
actual intent of this feature is that review-run behavior should require an agent explicitly
assigned as a code reviewer — the PR link says *which* PR, the agent's role says *whether this is
a review run at all*. This was a gap in the original design spec, not a scope change the user is
choosing to add; the spec's "Detection: a PR link in the task, not an agent role" section (line 27
of the spec as originally written) is corrected by Task 13 below, which also updates that section
in place.

Tasks 11–13 add this gate. They run through the same subagent-driven-development loop as Tasks
1–10 (fresh implementer, task review, fix loop) before the branch is considered ready.

### Task 11: `Agent.role` field — domain type, DB schema, migration, repository

**Files:**
- Modify: `packages/core/src/domain.ts` (add `AgentRole` type and `Agent.role` field)
- Modify: `packages/core/src/models.ts` or wherever `isValidOverflowPolicy`/`isValidModelId` live
  (find them via `grep -rn "isValidOverflowPolicy" packages/core/src` — add a sibling
  `isValidAgentRole` next to whichever one of those it's colocated with)
- Modify: `packages/db/src/schema.ts` (add `agentRoleEnum`, add `role` column to `agents` table)
- Create: `packages/db/drizzle/00XX_<auto-generated-name>.sql` (via `drizzle-kit generate`, do not
  hand-write)
- Modify: `packages/db/src/repositories/agents.ts` (`toAgent`, `NewAgentInput`, `AgentPatch`,
  `createAgent`, `updateAgent`)
- Test: `packages/db/src/__tests__/repositories/agents.test.ts` (find the existing file — extend
  it, do not create a new one)
- Test: wherever `isValidOverflowPolicy` already has unit tests — add `isValidAgentRole` cases
  alongside them

**Interfaces:**
- Produces: `AgentRole = "developer" | "reviewer"` exported from `@agentfactory/core`, added as a
  **required** (non-optional) field on the `Agent` domain interface: `role: AgentRole`. Every
  existing agent gets `"developer"` — that is the correct default, since "developer" describes
  every agent created before this feature existed.
- Produces: `isValidAgentRole(value: unknown): value is AgentRole`, same shape/location/naming
  convention as the existing `isValidModelId`/`isValidOverflowPolicy` guards.
- Produces: `NewAgentInput.role?: AgentRole` (optional — server applies the DB default when
  omitted) and `AgentPatch` gains `role` to its `Pick<Agent, ...>` list, both in
  `packages/db/src/repositories/agents.ts`.

- [ ] **Step 1: Add `AgentRole` and `Agent.role` to the domain type**

In `packages/core/src/domain.ts`, next to the existing `export type AgentMode = "manual" |
"automatic";` (around line 95), add:
...
Add `role: AgentRole;` to the `Agent` interface (around line 123), directly after `mode:
AgentMode;`.

- [ ] **Step 2: Add `isValidAgentRole`**

Find the existing `isValidModelId`/`isValidOverflowPolicy` functions with `grep -rn
"isValidOverflowPolicy" packages/core/src`. In the same file, next to `isValidOverflowPolicy`, add:
...
Add unit tests for it wherever `isValidOverflowPolicy`'s own tests live (same file/`describe`
block), asserting: `"developer"` → true, `"reviewer"` → true, `"manual"` → false, `undefined` →
false, `123` → false.

- [ ] **Step 3: Add the DB column**

In `packages/db/src/schema.ts`, next to `export const agentModeEnum = pgEnum("agent_mode",
["manual", "automatic"]);`, add:

```ts
export const agentRoleEnum = pgEnum("agent_role", ["developer", "reviewer"]);
```

In the `agents` table definition, directly after the `mode: agentModeEnum("mode").notNull()
.default("manual"),` line, add:
...
Run: `pnpm --filter @agentfactory/db exec drizzle-kit generate`
Expected: a new `packages/db/drizzle/00XX_*.sql` file adding the `agent_role` enum type and the
`agents.role` column with `DEFAULT 'developer'` and `NOT NULL`. Read the generated SQL to confirm
this before proceeding — do not hand-edit it.

- [ ] **Step 5: Wire the repository**

In `packages/db/src/repositories/agents.ts`:
- `toAgent`: add `role: row.role,` to the returned object.
- `NewAgentInput`: add `role?: AgentRole;` (import `AgentRole` from `@agentfactory/core`).
- `createAgent`: in the `.values({...})` call, add `role: input.role ?? "developer",`.
- `AgentPatch`: add `"role"` to the `Pick<Agent, ...>` union.
- `updateAgent`: add `if (patch.role !== undefined) values.role = patch.role;`.

- [ ] **Step 6: Extend the repository test**

In `packages/db/src/__tests__/repositories/agents.test.ts` (read the existing file first to match
its fixture/setup style), add two cases: (a) `createAgent` with no `role` in the input defaults to
`"developer"`; (b) `createAgent` with `role: "reviewer"` persists `"reviewer"`, and `updateAgent`
can change an existing agent's role from `"developer"` to `"reviewer"` and back.

- [ ] **Step 7: Typecheck, test, migrate, commit**

Run: `pnpm --filter @agentfactory/core typecheck && pnpm --filter @agentfactory/db typecheck`
Expected: PASS

Run: `pnpm --filter @agentfactory/db exec drizzle-kit migrate` (applies the new migration to the
local dev/test Postgres — needed before `pnpm test:db` can pass)
Expected: PASS

Run: `pnpm test:db -- agents`
Expected: PASS, including the two new cases

```bash
git add packages/core/src/domain.ts packages/db/src/schema.ts packages/db/drizzle/ packages/db/src/repositories/agents.ts packages/db/src/__tests__/repositories/agents.test.ts
git commit -m "feat(core,db): add Agent.role (developer|reviewer)"
```

(If `isValidAgentRole` and its tests live in a different file than `domain.ts`, `git add` that
file too.)

---

### Task 12: API validation and web UI — role selector

**Files:**
- Modify: `apps/web/src/app/api/agents/route.ts` (validate `body.role` on create)
- Modify: `apps/web/src/app/api/agents/[agentId]/route.ts` (validate `body.role` on update)
- Modify: `apps/web/src/lib/mock/context.tsx` (`NewAgentInput` type, `updateAgent`'s patch type)
- Modify: `apps/web/src/components/AgentFormModal.tsx` (`AgentFormValues`, role selector UI)
- Modify: `apps/web/src/app/(app)/agents/[agentId]/page.tsx` (pass `agent.role` into the edit
  modal's `initial`)
- Modify: `apps/web/src/app/(app)/teams-v2/page.tsx` (`NewAgentPanel`: role selector UI, include
  in the `createAgent(...)` call)
- Modify: `apps/web/src/lib/i18n/dictionaries/en.ts` (new keys, see Step 4)
- Test: `apps/web/src/components/__tests__/AgentFormModal.test.tsx` if it exists (`find
  apps/web/src/components/__tests__ -iname "*AgentFormModal*"` — extend it; if it doesn't exist,
  skip rather than inventing a new broad test file)

**Interfaces:**
- Consumes: `AgentRole`, `isValidAgentRole` from `@agentfactory/core` (Task 11).
- Produces: `AgentFormValues.role: AgentRole` (`apps/web/src/components/AgentFormModal.tsx`) —
  Task 13 does not depend on this, but keep it exact since it's a public component prop shape.

- [ ] **Step 1: Validate `role` in both API routes**

In `apps/web/src/app/api/agents/route.ts`'s `POST`, next to the existing `if (body.model !==
undefined && !isValidModelId(body.model))` check, add:
...
Add the same block to `apps/web/src/app/api/agents/[agentId]/route.ts`'s `PATCH`, in the same
position relative to its existing `isValidModelId`/`isValidOverflowPolicy` checks. Import
`isValidAgentRole` from `@agentfactory/core` in both files.

- [ ] **Step 2: Thread `role` through the mock/context data layer**

In `apps/web/src/lib/mock/context.tsx`:
- The local `NewAgentInput` interface (around line 35): add `role?: AgentRole;` and import
  `AgentRole` from `@agentfactory/core` in the existing type-only import at the top of the file.
- `updateAgent`'s patch parameter type: add `"role"` to the `Partial<Pick<Agent, ...>>` union
  (currently `"name" | "description" | "systemPrompt" | "mode" | "areaMap" | "defaultCodebase"`).

- [ ] **Step 3: Role selector in the edit modal (`AgentFormModal`)**

In `apps/web/src/components/AgentFormModal.tsx`:
- `AgentFormValues`: add `role: AgentRole;` and import `AgentRole` from `@agentfactory/core`
  alongside the existing `AgentMode` import.
- Add `const [role, setRole] = useState<AgentRole>(initial?.role ?? "developer");`
- Include `role,` in the object passed to `onSubmit` (find where `mode,` is already included).
- Add a selector UI block immediately after the existing mode selector block (the one with
  `{t("agentForm.modeLabel")}` / the two `manual`/`automatic` buttons), following the identical
  two-button pattern:
...
In `apps/web/src/lib/i18n/dictionaries/en.ts`, in the `agentForm` section (next to the existing
`modeLabel`/`modeManual`/`modeAutomatic`/`modeManualHelp`/`modeAutomaticHelp` keys), add:
...
roleReviewerHelp: "Also eligible to run PR-review tasks: when a task assigned to this agent links a GitHub PR, it checks out the PR and posts a real review to GitHub instead of running as a normal dev task.",
```

In the `teamsV2` section, add one key for the create-panel's role label:

```ts
agentRoleLabel: "Role",
```

(The create panel reuses `agentForm.roleDeveloper`/`roleReviewer`/`roleDeveloperHelp`/
`roleReviewerHelp` for the button labels and help text — only the section label itself needs a
`teamsV2`-namespaced key, matching how `agentModeLabel` is namespaced there today while the button
labels themselves come from `common.automatic`/a literal string.)

- [ ] **Step 5: Pass `role` through the edit page**

In `apps/web/src/app/(app)/agents/[agentId]/page.tsx`, add `role: agent.role,` to the `initial={{
...}}` object passed to `<AgentFormModal>` (`onSubmit` already forwards the full `values` object
to `updateAgent`, so no change needed there beyond Step 2's type widening).

- [ ] **Step 6: Role selector in the create panel (`teams-v2` `NewAgentPanel`)**

In `apps/web/src/app/(app)/teams-v2/page.tsx`'s `NewAgentPanel`:
- Add `const [role, setRole] = useState<AgentRole>("developer");` (import `AgentRole` from
  `@agentfactory/core`).
- Add `role` to the object passed to `createAgent({...})` in `handleCreate`.
- Add a selector block matching the existing agent-mode selector block in the same component
  (same two-button pattern, `{t("teamsV2.agentRoleLabel")}` as the label, `t("agentForm
  .roleDeveloper")`/`t("agentForm.roleReviewer")` as button text, `t("agentForm
  .roleDeveloperHelp")`/`t("agentForm.roleReviewerHelp")` as the help paragraph), placed directly
  after the existing mode selector block.
...
Run: `pnpm --filter @agentfactory/web test -- AgentFormModal` (if that test file exists; otherwise
run the broader `pnpm test:unit -- agents` filter and confirm nothing broke)
Expected: PASS

```bash
git add apps/web/src/app/api/agents/route.ts "apps/web/src/app/api/agents/[agentId]/route.ts" apps/web/src/lib/mock/context.tsx apps/web/src/components/AgentFormModal.tsx "apps/web/src/app/(app)/agents/[agentId]/page.tsx" "apps/web/src/app/(app)/teams-v2/page.tsx" apps/web/src/lib/i18n/dictionaries/en.ts apps/web/src/components/__tests__/AgentFormModal.test.tsx
git commit -m "feat(web): add a Developer/Reviewer role selector to agent create and edit"
```

(Omit any test file from the `git add` that doesn't exist.)

---

### Task 13: Gate PR-review detection on `agent.role === "reviewer"`

**Files:**
- Modify: `apps/worker/src/worker.ts` (the review-run detection gate)
- Modify: `docs/superpowers/specs/2026-09-15-pr-review-github-comments-design.md` (correct the
  "Detection" bullet)
- Test: `apps/worker/src/__tests__/worker.test.ts` (find the existing review-run-detection tests —
  `grep -n "prRef\|parsePullRequestReference\|review run" apps/worker/src/__tests__/worker.test.ts`
   — extend them)

**Interfaces:**
- Consumes: `agent.role` (`AgentRole`, Task 11) — `agent` is already in scope at the detection
  site in `worker.ts` (it's loaded earlier in the same function, before the `prRef` line).

- [ ] **Step 1: Read the current detection site**

Read `apps/worker/src/worker.ts` around the comment "Detection is live parsing of the task
description on every run" (search for `parsePullRequestReferenceAcrossProviders`). The current
code is:
...
Change the detection to also require the agent be a reviewer, and make sure a PR link on a
non-reviewer agent's task is treated as inert text (falls through to the normal dev-task /
chat-only path below) rather than being silently dropped with no explanation available to
someone debugging why a task with a PR link didn't post a review:
...
(Only the `const prRef = ...` line changes — the `if (prRef && task) {` line and everything inside
that block stays exactly as-is, since `prRef` is simply `undefined` for a non-reviewer agent and
the existing `} else if (task?.codebase) { ... }` / final chat-only branch already handles that
case correctly — a task with a PR link and a non-reviewer agent runs exactly like a task whose
description happens to contain unrelated text.)

Update the comment above this line (currently "Detection is live parsing of the task description
on every run — never a persisted flag on Task (see the design spec): retargeting a task at a
different PR, or removing the link entirely, takes effect on the next run with nothing to
migrate.") to also note the role requirement, e.g. append: "A review run additionally requires the
run's agent to have `role: \"reviewer\"` — an ordinary dev agent's task is never hijacked into a
review run just because its description happens to mention a PR link."

- [ ] **Step 3: Update the design spec**

In `docs/superpowers/specs/2026-09-15-pr-review-github-comments-design.md`, replace the bullet
"**Detection: a PR link in the task, not an agent role.**" (and its body) with a corrected version
stating: detection requires BOTH the assigned agent having `role: "reviewer"` AND a GitHub PR link
in `task.description` (same live-regex mechanism, `RegExp.exec`'s first-match behavior still gives
"first PR link wins" among multiple links) — the agent's role is what makes "post a real review to
GitHub" the correct behavior instead of a normal task run; the PR link is only needed to know
*which* PR. Keep the rest of that section's content (no new task-kind field, no persisted field on
`Task`, existing agent picker unchanged) — only the "not an agent role" framing and its rationale
change, since that's exactly what's now added.

- [ ] **Step 4: Add tests**

In `apps/worker/src/__tests__/worker.test.ts`, add test cases (read the existing review-run tests
first to match their fixture/mock style — likely a `fakeSandbox` + stubbed `ScmProvider` +
in-memory task/agent fixtures):
1. Task description has a PR link, agent has `role: "developer"` → the run does NOT check out a
   PR or call `postReview`; it proceeds down the normal dev-task or chat-only path (assert
   `checkoutPullRequest`/`postReview` were never called).
2. Task description has a PR link, agent has `role: "reviewer"` → the run DOES check out the PR
   and (assuming a valid structured review) calls `postReview` (this path should already be
   covered by Task 7's existing tests — if so, update the existing fixtures' agent role to
   `"reviewer"` explicitly rather than relying on an implicit default, so the test still makes
   sense read in isolation; don't duplicate the whole scenario if it already exists and passes
   once the fixture's role is set correctly).
3. Task description has no PR link, agent has `role: "reviewer"` → the run behaves as a normal
   dev-task/chat-only run (this should already work with no code change — the test just confirms
   a reviewer-role agent isn't otherwise different from a developer-role agent).

- [ ] **Step 5: Typecheck, test, commit**

Run: `pnpm --filter @agentfactory/worker typecheck`
Expected: PASS except the pre-existing unrelated `eval-judge.test.ts(995,79)` error (confirmed
present before this branch's first commit — do not attempt to fix it as part of this task)

Run: `pnpm test:unit -- worker`
Expected: PASS

```bash
git add apps/worker/src/worker.ts docs/superpowers/specs/2026-09-15-pr-review-github-comments-design.md apps/worker/src/__tests__/worker.test.ts
git commit -m "fix(worker): require agent.role === reviewer for PR-review runs, not just a PR link in the description"
```

- [ ] **Step 6: Re-run full-workspace verification**

Same as Task 10 — `pnpm typecheck && pnpm lint && pnpm test:unit && pnpm test:db && pnpm build` —
to confirm nothing regressed across the whole branch after Tasks 11–13.
