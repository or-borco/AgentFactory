...
export const RETROSPECTIVE_SYSTEM_PROMPT = [
  ...
  "",
  "A lesson must be durable, not task-specific. The agent keeps working in the same repositories, so how to",
  "work in one (which commands and scripts to run, setup steps, team conventions) is durable even when it",
  "holds only there. Where a piece of code lives, or what one change needed, is task-specific:",
  "- durable: \"Release notes here are for end users: no commit hashes, file names or function names\"",
  "- durable: \"Tests here run with `make check`; `npm test` is not set up\"",
