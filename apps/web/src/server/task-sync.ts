...
// it must never be why a run couldn't happen. Provider errors, a missing connection, a deleted
// issue, and an unset externalRef all resolve to "not stale"; only a successful fetch that shows
// real drift returns `stale: true`. The whole body is wrapped so this function itself can never
// throw — a Jira outage must read as "nothing to worry about," never as "block everything
// downstream."
