...
// Wrapped in React's cache() so the root layout (theme stamp) and (app)/layout.tsx (auth guard +
// AuthProvider) share one DB call per request instead of doubling the hit — cache() memoizes per
// render pass, so this is safe across the two independent layout invocations.
